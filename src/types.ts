...
export type QCMoveChoice =
  ...
  | { type: "split"; from: number; targetA: number; targetB: number; phaseQuarters?: number; splitFraction?: number; _forceMeasurement?: "m0" | "m1" }
  | { type: "merge"; sourceA: number; sourceB: number; to: number; phaseQuarters?: number; splitFraction?: number; _forceMeasurement?: "m0" | "m1" };

// phaseQuarters: optional phase rotation of the moving piece, in quarter
// turns. Alchemy League allows whole quarters, Universal QC any angle; the
// engine rejects what the active rules forbid.
// splitFraction (Universal QC): the strength of a split's first iSwap, the
// source to targetA, or of a merge's last iSwap, sourceA to the target. A merge
// at strength f undoes a split at strength f. Absent means the standard 0.5.
